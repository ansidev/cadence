...
//
//	mockgen -package=metrics -destination=client_mock.go -self_package=github.com/uber/cadence/common/metrics github.com/uber/cadence/common/metrics Client
//
...
func (m *MockClient) AddCounter(scope ScopeIdx, counter MetricIdx, delta int64) {
	m.ctrl.T.Helper()
...
func (m *MockClient) IncCounter(scope ScopeIdx, counter MetricIdx) {
	m.ctrl.T.Helper()
...
func (m *MockClient) RecordHistogramDuration(scope ScopeIdx, timer MetricIdx, d time.Duration) {
	m.ctrl.T.Helper()
...
func (m *MockClient) RecordTimer(scope ScopeIdx, timer MetricIdx, d time.Duration) {
	m.ctrl.T.Helper()
...
func (m *MockClient) Scope(scope ScopeIdx, tags ...Tag) Scope {
	m.ctrl.T.Helper()
	varargs := []any{scope}
	for _, a := range tags {
...
func (m *MockClient) StartTimer(scope ScopeIdx, timer MetricIdx) tally.Stopwatch {
	m.ctrl.T.Helper()
...
func (m *MockClient) UpdateGauge(scope ScopeIdx, gauge MetricIdx, value float64) {
	m.ctrl.T.Helper()
